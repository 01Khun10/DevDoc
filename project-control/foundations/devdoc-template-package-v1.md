...
# 16. Implementation Status (verified against the repo, 2026-07-21) [ADDED]

Everything above this line is the original package spec, unedited. This section reports what's actually true in the code today, checked directly — not assumed. Per `AGENTS.md` §3 ("when sources conflict, report the conflict, do not silently choose"), findings that require a decision are flagged, not resolved here.

## Verified matching the spec

- **Folder structure** — `backend/src/data/templates/{standard,academic,company}/*Template.js` + `profiles.js` matches §5 exactly.
- **3 profiles seeded** with the exact codes this package requires: `STANDARD_SOFTWARE`, `ACADEMIC_PROJECT`, `COMPANY_SOFTWARE`.
- **12 templates seeded** (4 document types × 3 profiles), template codes match §3 exactly (`STD_SRS`, `ACAD_SDS`, `COMP_STP`, etc.).
- **`DocumentType` enum** in `prisma/schema.prisma` is exactly `SCOPE | SRS | SDS | STP`, matching §2.
- **`TemplateSection` Prisma fields** match the §6 field list exactly, including `validationTag`, `guidanceText`, `exampleText`, `placeholderText`, `parentSectionId`.
- **`RuleCategory` enum** is exactly `C | Q | CN | T | D | SEC`, matching §8's category codes.
- **ERD placement rule (§7) is honored** — `grep`-confirmed: "ERD" / "Data Design" / "Database Design" sections exist only in the three `*SdsTemplate.js` files, never in Scope, SRS, or STP templates.
- **Doc-Linter required-section check exists** — `validationChecks.js` filters sections by `isRequired` and checks completion, which is the mechanism §14's readiness checklist calls for.
- **Seed wiring works** — `prisma/seed.js` imports `{ profiles, templates, validationRules }` and upserts all three, so `npx prisma db seed` populates this correctly.

## Real gaps (content, not code)

- **[MEDIUM] `exampleText` is `null` in every section, in all 12 templates, with zero exceptions** (verified: `grep -c "exampleText: \`" ` across every template file returns 0). §4's Frontend Display Rule says the editor should show an Example "if available" — it is never available. The `TemplateSection` object shape in §12 explicitly includes it as real content, not an afterthought. This is an authoring task, not a schema task — the field exists and works, it's just empty everywhere.
- **[MEDIUM] `guidanceText` is machine-generated boilerplate, not authored guidance.** The actual generator (`standard/stdSrsTemplate.js` and siblings) produces text like *"Write clear requirement-focused details for the purpose"* for every section via a template-string helper — not the specific, real guidance §12's object shape implies (*"Clear help text shown to the user in the editor"*). Structurally correct, substantively thin.

## Open conflict — RESOLVED 2026-07-21

- **[RESOLVED]** §8's Validation Rule Naming Convention specifies profile+template-prefixed rule codes (`STD-SRS-C-001`, `ACAD-SDS-D-001`). The actual implementation (`backend/src/data/templates/validationRules.js`) uses a **different, simpler scheme**: ~18 base rules with short domain codes (`DOC-001`, `SEC-001`, ...), each seeded once per profile with per-profile `severityOverrides` rather than 12× duplicated profile-specific rules. **Decided by the user: the implemented `DOC-001`-style shared-code scheme is canonical.** §8 above is superseded by this decision — kept unedited as historical record of the original spec, per this file's own precedence note (PRD wins, `foundations/` gets a pointer, never silently rewritten). The code does not need to change; `project-control/prds/02-PRD-backend.md` §8 now documents the shared-code scheme as the real convention. See `CHANGELOG.md`.

## Frontend Display Rule violation (own finding, from earlier work in this project)

- **[HIGH]** §4 is explicit: `ruleCode` must not be shown directly in the UI — it's a backend/Doc-Linter field. `frontend/src/pages/ValidationEngine.jsx` and `frontend/src/pages/SharedReport.jsx` currently render `{r.ruleCode}` as raw visible text (e.g. a user sees literal `"DOC-001"` next to a finding). This was built without having read this package — it's a real defect against this project's own stated rule, not a hypothetical one. Fix: replace the raw code with `ruleLabels[ruleCode]` (a pattern `ValidationResultCard.jsx` already uses correctly elsewhere in the same codebase) or hide it behind a "technical details" disclosure. Not fixed in this pass — flagged for the next frontend touch on those two files.

## Net assessment

The *structure* of this package — profiles, templates, section schema, ERD placement, required-section checking — is faithfully implemented, better than most of the frontend audits found elsewhere in this project. The gaps are exactly where you'd predict: the parts that require actual writing (example content, real per-section guidance) are thin, and one convention (rule-code naming) drifted between spec and implementation without either side being updated to match the other.

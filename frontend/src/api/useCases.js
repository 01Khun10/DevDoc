...
import { createUseCase, deleteUseCase, listUseCases, updateUseCase } from "../services/useCaseService";
...
export function useDeleteUseCase(projectId, options = {}) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (useCaseId) => deleteUseCase(projectId, useCaseId),
    ...options,
    onSuccess: (...args) => {
      queryClient.invalidateQueries({ queryKey: ["projects", projectId, "use-cases"] });
